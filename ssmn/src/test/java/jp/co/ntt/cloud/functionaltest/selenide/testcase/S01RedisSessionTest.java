...
public class S01RedisSessionTest {
    ...
    public void session01RedisTest() throws IOException, InterruptedException, BuildException, ClassNotFoundException {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                ViewCartPage viewCartPage = open(applicationContextUrl
                        + WebPagePathConstants.ORDER_FORM, LoginPage.class)
                                .loginOrderForm("0000000002", "aaaaa11111")
                ...
                for (CartItem cartItem : cart.getCartItems()) {
                    viewCartPage.getCartItemName(index).shouldHave(exactText(
                            cartItem.getProduct().getName()));
                    viewCartPage.getCartItemPrice(index).shouldHave(text(
                            moneyUnitToString(cartItem.getProduct()
                                    .getPrice())));
                    index++;
                }
                ...
                index = 0;
                for (CartItem cartItem : cart.getCartItems()) {
                    orderConfirmPage.getCartItemName(index).shouldHave(
                            exactText(cartItem.getProduct().getName()));
                    orderConfirmPage.getCartItemPrice(index).shouldHave(text(
                            moneyUnitToString(cartItem.getProduct()
                                    .getPrice())));
                    orderConfirmPage.getCartItemQuantity(index).shouldHave(
                            exactText(String.valueOf(cartItem.getQuantity())));
                    index++;
                }
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                IndexPage sessionIndexPage = open(applicationContextUrl
                        + WebPagePathConstants.SESSION_ISAUTHENTICATED,
                        LoginPage.class).loginIsAuthenticated("0000000002",
                                "aaaaa11111");
                ...
                open(applicationContextUrl
                        + WebPagePathConstants.SESSION_ISAUTHENTICATED,
                        LoginPage.class).getMessage().shouldHave(exactText(
