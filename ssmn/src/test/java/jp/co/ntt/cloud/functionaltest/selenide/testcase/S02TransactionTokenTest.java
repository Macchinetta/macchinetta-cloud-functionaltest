...
public class S02TransactionTokenTest {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                CreateOutputPage createOutputPage = open(applicationContextUrl
                        + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink1().clickBtn1();
    ...
    public void testSSMN0101002() {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                CreateOutputPage createOutputPage = open(applicationContextUrl
                        + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink1().clickBtn2();
    ...
    public void testSSMN0101003() {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                CreateOutputPage createOutputPage = open(applicationContextUrl
                        + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink1().clickBtn3();
    ...
    public void testSSMN0101004() {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                CreateOutputPage createOutputPage = open(applicationContextUrl
                        + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink1().clickBtn4();
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                Flow1Step2Page flow1Step2Page = open(applicationContextUrl
                        + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink2().clickBtnFlow1();
                ...
                String currentToken = flow1Step2Page.getTransactionToken()
                        .getValue();
                ...
                flow1Step3Page.getResult().shouldHave(matchText("^(?!.*"
                        + currentToken.split("~")[2] + ").*(?=" + currentToken
                                .split("~")[0] + "~" + currentToken.split(
                                        "~")[1] + ").*$"));
                ...
                flow1Step3Page.clickBtnCheck().getTransactionToken().shouldHave(
                        value(currentToken));
                ...
                flow1Step3Page.clickBtnEnd().getTransactionToken()
                        .shouldNotHave(exist);
            ...
            try {
                ...
                Flow1NamespaceStep2Page flow1NamespaceStep2Page = open(
                        applicationContextUrl
                                + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink5().clickBtnFlow1();

                // アサート:hidden項目「_TRANSACTION_TOKEN」が入っていること（作成されている）
                flow1NamespaceStep2Page.getTransactionToken().shouldNotHave(
                        empty);

                // 証跡取得
                screenshot("S02TransactionTokenTest-testSSMN0102002-001");

                String currentToken = flow1NamespaceStep2Page
                        .getTransactionToken().getValue();
                ...
                currentToken = flow1NamespaceStep3Page.getTransactionToken()
                        .getValue();
    ...
    public void testSSMN0102003() {
        ...
        Flow2Step2Page flow2Step2Page = open(applicationContextUrl
                + WebPagePathConstants.TRANSACTIONTOKEN, IndexPage.class)
                        .clickLink2().clickBtnFlow2();
    ...
    public void testSSMN0102005() {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                // アサート:BEGINメソッドで作られたtransaction tokenと、INメソッドで作られたtransaction
                // tokenの比較を行い、name、keyは一致し、valueは一致していないこと(特定部分のみ更新されていること)
                flow1Step2Page.getResult().shouldHave(matchText("^(?!.*"
                        + currentToken.split("~")[2] + ").*(?=" + currentToken
                                .split("~")[0] + "~" + currentToken.split(
                                        "~")[1] + ").*$"));
                ...
                // アサート:ENDメソッドに対するリクエストのレスポンスにhidden項目「_TRANSACTION_TOKEN」が入っていないこと（破棄されている）
                flow1Step2Page.clickBtnEnd().getTransactionToken()
                        .shouldNotHave(exist);
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                FlowAllStep1Page flowAllStep1Page = open(applicationContextUrl
                        + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink2().clickBtnFlow5_1();
                ...
                String currentToken = flowAllStep1Page.getTransactionToken()
                        .getValue();

                // テスト実行(BEGINのついているメソッドに対するリクエスト)
                Flow1Step2Page flow1Step2Page = flowAllStep1Page
                        .clickBtnFlow5_2();
                ...
                // tokenの比較を行い、name、keyは一致し、valueは一致していないこと(特定部分のみ更新されていること)
                flow1Step3Page.getResult().shouldHave(matchText("^(?!.*("
                        + currentToken.split("~")[1] + "|" + currentToken.split(
                                "~")[2] + ").*(?=" + currentToken.split("~")[0]
                        + ")).*$"));
    ...
    public void testSSMN0102007() {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                flow1Step2Page.clickBtnEndError().getTransactionToken()
                        .shouldNotHave(exist);
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                open(applicationContextUrl
                        + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink2().clickBtnFlow3().getH()
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                Flow1Step2ToDifferentNamespacePage flow1Step2ToDifferentNamespacePage = open(
                        applicationContextUrl
                                + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink2().clickBtnFlow7();

                // アサート:hidden項目「_TRANSACTION_TOKEN」が入っていること（作成されている）
                flow1Step2ToDifferentNamespacePage.getTransactionToken()
                        .shouldNotHave(empty);
                ...
                flow1Step2ToDifferentNamespacePage.clickBtnIn().getH()
                        .shouldHave(exactText("Transaction Token Error"));
    ...
    public void testSSMN0102011() {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                open(applicationContextUrl
                        + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink2().clickBtnFlow4().getH()
                                .shouldHave(exactText(
                                        "Transaction Token Error"));
    ...
    public void testSSMN0102012() {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                flow1Step2ToDifferentNamespacePage.clickBtnEnd().getH()
                        .shouldHave(exactText("Transaction Token Error"));
    ...
    public void testSSMN0102016() {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                open(applicationContextUrl
                        + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink2().clickBtnFlow8().getH()
                                .shouldHave(exactText(
                                        "Transaction Token Error"));
            ...
            try {
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    WebDriverRunner.getWebDriver().switchTo().window(
                            new LinkedList<String>(WebDriverRunner
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                CustomStoreSizeNextPage customStoreSizeNextPage = open(
                        applicationContextUrl
                                + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink3().clickBtnBegin1Other();
                ...
                // 新規タブを開く
                IndexPage indexPage = customStoreSizeNextPage
                        .clickOpenNewWindow();
                WebDriverRunner.getWebDriver().switchTo().window(
                ...
                // テスト実行(BEGINのついているメソッドに対するリクエスト)
                customStoreSizeNextPage = indexPage.clickLink3()
                        .clickBtnBegin1();
                ...
                WebDriverRunner.getWebDriver().switchTo().window(
                        conflictWindow);
                customStoreSizeNextPage.clickBtnIn1().getH().shouldHave(
                        exactText("Transaction Token Error"));
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                CustomStoreSizeNextPage customStoreSizeNextPage = open(
                        applicationContextUrl
                                + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink3().clickBtnBegin2();
                ...
                // アサート:1回目のタブに移動してサブミットするとトークンエラーになっていること
                WebDriverRunner.getWebDriver().switchTo().window(mainWindow);
                customStoreSizeNextPage.clickBtnIn2().getH().shouldHave(
                        exactText("Transaction Token Error"));
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                CustomStoreSizeNextPage customStoreSizeNextPage = open(
                        applicationContextUrl
                                + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink3().clickBtnBegin3();
                ...
                // アサート:1回目のタブに移動してサブミットするとトークンエラーになっていること
                WebDriverRunner.getWebDriver().switchTo().window(mainWindow);
                customStoreSizeNextPage.clickBtnIn3().getH().shouldHave(
                        exactText("Transaction Token Error"));
    ...
    public void testSSMN0103006() {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                flow1Step2Page.clickRedo1().clickBtnFlow1().getResult()
    ...
    public void testSSMN0104001() {
        ...
        for (int retryCount = 0; retryCount < 100; retryCount++) {
            try {
                ...
                open(applicationContextUrl
                        + WebPagePathConstants.TRANSACTIONTOKEN,
                        IndexPage.class).clickLink2().clickBtnFlow6()
                                .getResult().shouldHave(matchText(
                                        "globalToken~[0-9a-z]{32}~[0-9a-z]{32}"));
