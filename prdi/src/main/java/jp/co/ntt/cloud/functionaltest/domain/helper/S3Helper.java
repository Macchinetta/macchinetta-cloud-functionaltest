...
public class S3Helper implements InitializingBean {
    ...
    /**
     * 指定されたバケット、キーのみを GET するポリシーを取得する。 {@link Effect} に <code>Deny</code> を指定することで、 <code>NotAction</code> に指定した操作、および
    ...
    private String getPolicy(String bucketName, String key) {
        ...
        Statement statement = new Statement(Statement.Effect.Allow).withActions(
                S3Actions.GetObject).withResources(new Resource(resource));
    ...
    /**
     * 指定されたポリシーを持つ一時的なロールを取得する。 署名付きURLには、そのURLを発行したロールのアクセスキー情報が含まれるため、 STSにて一時的なロールを作成する。 ダウンロード対象のバケットポリシーにて、この一時ロールによるアクセスが
    ...
    private AmazonS3 getClient(String policy) {
        ...
        STSAssumeRoleSessionCredentialsProvider credentialsProvider = new STSAssumeRoleSessionCredentialsProvider.Builder(roleArn, roleSessionName)
                ...
                .withScopeDownPolicy(policy).build();
