...
public class S3NoticeMessageListener {
    ...
    private void createRecord(S3EventNotificationRecord eventMsg) {
        ...
        DateTime uploadDate = eventMsg.getEventTime().withZone(DateTimeZone.forID("Asia/Tokyo"));
        String strUploadDate = uploadDate.toString(DateTimeFormat.forPattern("yyyy-MM-dd"));
    ...
    /**
     * 現在値(curSequencer)と今回値(newSequencer)を比較して，
     * 今回値の方が時系列的に新しいデータであることを確認する。
     * 確認は桁数を揃えた後に辞書式比較によって大きい方を新しいデータと判定する。
     *
    ...
    /**
     * オブジェクトキー（"[UserId]-[FileName]"のフォーマットの想定）を
     * {[UserId],[FileName]}の文字列配列に分割する。
     *
    ...
    private String[] dataSplit(String objectKey) {
        ...
        if (objectKey.contains("-")) {
            ...
            return new String[]{
                    objectKey.substring(0, dividePos),
                    objectKey.substring(dividePos+1)};
        } else {
