...
public class SearchSharedServiceImpl implements SearchSharedService {
    ...
    public List<FileMetaData> doSearch() {
        List<FileMetaData> result = dbMapper.scan(FileMetaData.class,
    ...
    public List<FileMetaData> doUserIdIndexSearch(String uploadUser,
        ...
        List<FileMetaData> indexResult = dbMapper.query(FileMetaData.class,
                queryExpression);
    ...
    public List<FileMetaData> doBucketNameIndexSearch(String bucketName) {
        ...
        DynamoDBQueryExpression<FileMetaData> queryExpression = new DynamoDBQueryExpression<FileMetaData>()
                .withIndexName("bucketName-size-index").withConsistentRead(
                        false) // グローバルセカンダリインデックスの場合は強い整合性のある読み込みをサポートしないためfalseを指定
