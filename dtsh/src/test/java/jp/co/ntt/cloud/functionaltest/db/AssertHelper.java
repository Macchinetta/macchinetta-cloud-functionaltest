...
public class AssertHelper {
    ...
    public static void assertViewMember(Member expectedMember,
            HelloPage helloPage) {
        assertEquals(expectedMember.getCustomerNo(), helloPage.getCustomerNo()
                .getText());
        assertEquals(expectedMember.getName(), helloPage.getName().getText());
        assertEquals(expectedMember.getFuriName(), helloPage.getFuriName()
                .getText());
    }
    ...
    public static void assertViewResvation(Reservation expectedReservation,
            HelloPage helloPage) {
        assertEquals(expectedReservation.getReserveNo(), helloPage
                .getReserveNo().getText());
        assertEquals(DateTimeFormat.forPattern("yyyy/MM/dd").print(
                expectedReservation.getReserveDate()), helloPage
                        .getReserveDate().getText());
        assertEquals(expectedReservation.getTotalFare(), Integer.valueOf(
                helloPage.getTotalFare().getText()));

        assertEquals(expectedReservation.getRepMember().getCustomerNo(),
                helloPage.getRepCustomerNo().getText());
        assertEquals(expectedReservation.getRepMember().getName(), helloPage
                .getRepName().getText());
        assertEquals(expectedReservation.getRepMember().getFuriName(), helloPage
                .getRepFuriName().getText());
    }
}
